...
- 본문 문장만이 아니라 **모든 위치**에 적용한다. AI 글은 다음 자리에 특히 em대시를 즐겨 끼우는데, 전부 금지다:
  - **헤딩/소제목**: ❌ `## 설계 1: DB를 두지 않는다 — S3 스토어` → ✅ 괄호나 콜론으로. `## 설계 1: DB를 두지 않는다(S3 스토어)` /
    `### 같은 기사인지 어떻게 아나: URL 정규화`
  - **굵은 라벨 + 설명 목록 항목**: ❌ `- **시드 폴백** — env가 없을 때…` → ✅ 콜론으로. `- **시드 폴백**: env가 없을 때…`
    (또는 `- **iOS.** App ID를 만들고…`처럼 마침표)
  - **표 셀**: ❌ `**optional** — 서버가 채울 거니까` → ✅ 괄호로. `**optional**(서버가 채울 거니까)`
  - **반전·여운을 노린 `— 고 생각했다`류**: ❌ `출시만 하면 된다 — 고 생각했다.` → ✅ 문장을 끊는다.
    `출시만 하면 된다. 그렇게 생각했다.`
- 범위·구간 표기(`2023–2024` 같은 en대시)나 코드 블록·인라인 코드 안의 `—`는 예외로 허용한다.
- 글을 추가/수정한 뒤 `grep "—"`로 본문에 em대시가 남지 않았는지 확인한다(코드 블록 주석의 `—`만 남아야 한다).
